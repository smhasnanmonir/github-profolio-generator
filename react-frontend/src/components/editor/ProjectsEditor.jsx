...
import { Plus, X, GripVertical, ExternalLink, Star, GitFork } from 'lucide-react';

export default function ProjectsEditor({ portfolio, updateProject, addProject, removeProject, moveProject, allData, onShowAddModal }) {
  ...
  const handleDrag = (index, e, data) => {
    ...
    const draggedY = data.node.getBoundingClientRect().top + data.node.offsetHeight / 2;
    ...
    itemRefs.current.forEach((ref, i) => {
      if (ref && i !== draggedIndex) {
        ...
        } else if (draggedY < itemCenterY && i < draggedIndex && i >= ghostIndex) {
  ...
  const handleDragStop = () => {
    if (draggedIndex !== null && ghostIndex !== null && draggedIndex !== ghostIndex) {
  ...
  const getItemStyle = (index) => {
    ...
    if (draggedIndex < ghostIndex && index > draggedIndex && index <= ghostIndex) {
      return { transform: 'translateY(-120px)', transition: 'transform 0.2s ease' };
    }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Drag to reorder your projects</p>
        </div>
        <button
      ...
      <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
        <p className="text-sm text-blue-300">
          💡 <strong>Tip:</strong> Grab the grip icon (⋮⋮) and drag projects to reorder them. Blue border shows the active project.
        </p>
      </div>
    </div>
  );
